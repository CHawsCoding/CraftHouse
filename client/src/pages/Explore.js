...
function Explore({ primaryColor }) {
  ...
  return (
    <div
      style={{ backgroundColor: primaryColor, color: "white", padding: "16px" }}
        ...
        <div className="grid grid-cols-1 mr-5 ml-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {DIYs &&
            DIYs.map((DIY, index) => (
              <div
                key={DIY._id}
                className="bg-gray-900 border-2 border-gray-800 rounded-lg shadow-lg transition transform duration-300 hover:scale-105 overflow-hidden"
              ...
              >
                ...
                <div className="p-6 border-t">
                  <h3 className="text-xl text-gray-500 font-semibold mb-2 text-center underline">
                    {DIY.title}
                  </h3>
                </div>
                <div className="text-center p-4">
                  ...
                  >
                    <h3 className="text-lg text-yellow-500 hover:text-yellow-600 font-semibold mb-2 text-center underline">
                      Learn More
                    </h3>
                  </Link>
                </div>
                ...
                <div className="flex justify-between px-6 py-4">
                  <SlLike
                    size={24}
                    className="text-white-600 hover:text-yellow-500 hover:scale-125 cursor-pointer"
                    onClick={() => handleLike(DIY._id)}
                  />
                  <SlDislike
                    size={24}
                    className="text-white-600 hover:text-red-500 hover:scale-125 cursor-pointer"
                    onClick={() => handleDislike(DIY._id)}
                  />
                  <HiOutlineSaveAs
                    size={24}
                    className="text-white-600 hover:text-yellow-500 hover:scale-125 cursor-pointer"
                    onClick={() => handleSave(DIY._id)}
                  />
                </div>
                ...
                <div className="relative">
                  <form
                    ...
                    onSubmit={(e) => handleComment(e, DIY._id)}
                  >
                    <input
                      type="text"
                  ...
                  {/* comments rendered from comments.js*/}
                </div>
              </div>
            ))}
        </div>
